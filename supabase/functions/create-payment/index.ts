...
serve(async (req) => {
    ...
    try {
        ...
        const SUPPORTED_PAYMENT_METHODS = ['OL', 'DA', 'LQRIS', 'NQRIS', 'BC'];
        if (!SUPPORTED_PAYMENT_METHODS.includes(payment_method)) {
            throw new Error(`Invalid payment method. Supported methods: ${SUPPORTED_PAYMENT_METHODS.join(', ')}`);
        }
        ...
        // Prepare the payload according to Duitku documentation
        const duitkuBody: any = {
            merchantCode: Deno.env.get('DUITKU_MERCHANT_CODE'), // From environment
            paymentAmount: amountInt, // Integer amount
            paymentMethod: payment_method, // From validated input
            merchantOrderId: merchantOrderId, // Generated order ID
            productDetails: `${plan.plan_name} Subscription`, // Product details from plan
            email: user.email, // From user data
            customerVaName: user.email?.split('@')[0] || 'Customer', // Customer name
            callbackUrl: Deno.env.get('DUITKU_CALLBACK_URL') || 'https://xasuqqebngantzaenmwq.supabase.co/functions/v1/payment-webhook', // Callback URL
            returnUrl: return_url || Deno.env.get('DUITKU_RETURN_URL') || 'http://localhost:5173/settings', // Return URL
            signature: generateSignature(merchantCode, merchantOrderId, amountInt, apiKey), // Generated signature
            expiryPeriod: 1440 // 24 hours in minutes
        }

        // Add accountLink for OVO LINK (OL) payment method as per documentation
        ...
        const { data: transaction, error: transError } = await supabase
            ...
            .insert({
                ...
                expired_at: new Date(Date.now() + 24 * 60 * 1000)
            })
            .select()
            .single()
